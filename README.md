...
- 장면 설명에서 카메라 구도, 매장·방송 환경, 동작·표정·성인 장면 태그를 더 세밀하게 인식하고, 불필요하게 넓은 일반 태그는 초안에서 제외하도록 규칙을 확장했습니다.
- 인원 프리셋을 적용할 때 중복 태그를 정리하고, 이미지 생성 취소나 오류 뒤에도 사용자가 직접 편집한 프롬프트가 유지되도록 개선했습니다.

### v1.0.17

- 태그 드래프트 규칙, 정렬, 기본 적용 위치를 `core.js` 공용 로직으로 모아 이후 규칙 추가 시 중복 수정 범위를 줄였습니다.
- `index.js`와 `renderer.js`가 같은 태그 분배/와일드카드 분배 헬퍼를 사용하도록 정리했습니다.
- Electron preload가 공용 태그 모듈을 정상 로드하도록 설정을 보완해 TXT 불러오기와 버튼 클릭이 멈추는 문제를 고쳤습니다.
